...
*/
	public class LeanAudio : object {
		...
		public static LeanAudioStream createAudioStream( AnimationCurve volume, AnimationCurve frequency, LeanAudioOptions options = null ){
			...
			int generatedWavePtsLength = LeanAudio.createAudioWave( volume, frequency, options);
			LeanAudio.createAudioFromWave( generatedWavePtsLength, options );
	...
	*/
		public static AudioClip createAudio( AnimationCurve volume, AnimationCurve frequency, LeanAudioOptions options = null ){
			...
			return LeanAudio.createAudioFromWave( generatedWavePtsLength, options );
		}
		...
		private static int createAudioWave( AnimationCurve volume, AnimationCurve frequency, LeanAudioOptions options ){
			...
			for(int i = 0; i < LeanAudio.PROCESSING_ITERATIONS_MAX; i++){
				...
				if(f<LeanAudio.MIN_FREQEUNCY_PERIOD)
					f = LeanAudio.MIN_FREQEUNCY_PERIOD;
				...
				if(passed + 0.5f*f>=time)
					break;
				if(listLength >= LeanAudio.PROCESSING_ITERATIONS_MAX-1){
					...
					LeanAudio.generatedWaveDistances[ distPoint ] = passed;
					//Debug.Log("distPoint:"+distPoint+" passed:"+passed);

					//list.Add( passed );
					//list.Add( i%2==0 ? -height : height );

					LeanAudio.longList[ listLength ] = passed;
					LeanAudio.longList[ listLength + 1 ] = i%2==0 ? -height : height;
				}
			...
			listLength += -2;
			LeanAudio.generatedWaveDistancesCount = listLength / 2;
		...
		private static AudioClip createAudioFromWave( int waveLength, LeanAudioOptions options ){
			float time = LeanAudio.longList[ waveLength - 2 ];
			float[] audioArr = new float[ (int)(options.frequencyRate*time) ];

			int waveIter = 0;
			float subWaveDiff = LeanAudio.longList[waveIter];
			float subWaveTimeLast = 0f;
			float subWaveTime = LeanAudio.longList[waveIter];
			float waveHeight = LeanAudio.longList[waveIter+1];
			for(int i = 0; i < audioArr.Length; i++){
				float passedTime = (float)i / (float)options.frequencyRate;
				if(passedTime > LeanAudio.longList[waveIter] ){
					subWaveTimeLast = LeanAudio.longList[waveIter];
					waveIter += 2;
					subWaveDiff = LeanAudio.longList[waveIter] - LeanAudio.longList[waveIter-2];
					waveHeight = LeanAudio.longList[waveIter+1];
		...
		AudioClip audioClip = AudioClip.Create("Generated Audio", lengthSamples, 1, options.frequencyRate, is3dSound, false);
			...
			if(options.useSetData){
				audioClip = AudioClip.Create("Generated Audio", lengthSamples, 1, options.frequencyRate, false, null, LeanAudio.OnAudioSetPosition);
