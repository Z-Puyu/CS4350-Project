...
namespace LeanTween.Examples.Scripts {
	public class GeneralAdvancedTechniques : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.alpha( this.avatarRecursive, 0f, 1f).setRecursive(true).setLoopPingPong();
			Framework.LeanTween.alpha( this.avatar2dRecursive, 0f, 1f).setRecursive(true).setLoopPingPong();
			Framework.LeanTween.alpha( this.wingPersonPanel, 0f, 1f).setRecursive(true).setLoopPingPong();
			...
			Framework.LeanTween.value( this.avatarMove, 0f, (float)this.movePts.Length-1, 5f).setOnUpdate((float val)=>{
				int first = (int)Mathf.Floor(val);
				int next = first < this.movePts.Length-1 ? first + 1 : first;
				float diff = val - (float)first;
				// Debug.Log("val:"+val+" first:"+first+" next:"+next);
				Vector3 diffPos = (this.movePts[next].position-this.movePts[first].position);
				this.avatarMove.transform.position = this.movePts[first].position + diffPos*diff;
			}).setEase(LeanTweenType.easeInOutExpo).setLoopPingPong();

			// move the pts
			for(int i = 0; i < this.movePts.Length; i++)
				Framework.LeanTween.moveY( this.movePts[i].gameObject, this.movePts[i].position.y + 1.5f, 1f).setDelay(((float)i)*0.2f).setLoopPingPong();


			// move objects at a constant speed
			for(int i = 0; i < this.avatarSpeed.Length; i++)
				Framework.LeanTween.moveLocalZ( this.avatarSpeed[i], (i+1)*5f, 1f).setSpeed(6f).setEase(LeanTweenType.easeInOutExpo).setLoopPingPong(); // any time you set the speed it overrides the time value
	
			// move around a circle at a constant speed
			for(int i = 0; i < this.avatarSpeed2.Length; i++){
				Framework.LeanTween.moveLocal( this.avatarSpeed2[i], i == 0 ? this.circleSm : this.circleLrg, 1f).setSpeed(20f).setRepeat(-1);
			}
