...
namespace LeanTween.Examples.Scripts {
	public class LogoCinematic : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.moveLocalX(this.tween, this.tween.transform.localPosition.x+15f, 0.4f).setEase(LeanTweenType.linear).setDelay(0f).setOnComplete( this.playBoom );

			// Drop Down tween down
			this.tween.transform.RotateAround(this.tween.transform.position, Vector3.forward, -30f);
			Framework.LeanTween.rotateAround(this.tween, Vector3.forward, 30f, 0.4f).setEase(LeanTweenType.easeInQuad).setDelay(0.4f).setOnComplete( this.playBoom );

			// Drop Lean In
			this.lean.transform.position += Vector3.up * 5.1f;
			Framework.LeanTween.moveY(this.lean, this.lean.transform.position.y-5.1f, 0.6f).setEase(LeanTweenType.easeInQuad).setDelay(0.6f).setOnComplete( this.playBoom );
		}
