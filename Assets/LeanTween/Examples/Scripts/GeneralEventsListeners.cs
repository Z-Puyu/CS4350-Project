...
namespace LeanTween.Examples.Scripts {
	public class GeneralEventsListeners : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.addListener(this.gameObject, (int)MyEvents.CHANGE_COLOR, this.changeColor);
			Framework.LeanTween.addListener(this.gameObject, (int)MyEvents.JUMP, this.jumpUp);
		}
		...
		void changeColor( LTEvent e ){
			...
			float distance = Vector3.Distance( tran.position, this.transform.position);
			Color to = new Color(Random.Range(0f,1f),0f,Random.Range(0f,1f));
			Framework.LeanTween.value( this.gameObject, this.fromColor, to, 0.8f ).setLoopPingPong(1).setDelay(distance*0.05f).setOnUpdate(
				(Color col)=>{
		...
		void OnCollisionEnter(Collision collision) {
			if(collision.gameObject.layer!=2)
				this.towardsRotation = new Vector3(0f, Random.Range(-180, 180), 0f);
		}
		...
		void OnMouseDown(){
			if(Input.GetKey( KeyCode.J )){ // Are you also pressing the "j" key while clicking
				...
				Framework.LeanTween.dispatchEvent((int)MyEvents.CHANGE_COLOR, this.transform); // with every dispatched event, you can include an object (retrieve this object with the *.data var in LTEvent)
			}
		}
	}
}
