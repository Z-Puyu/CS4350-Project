...
namespace LeanTween.Examples.Scripts {
	public class PathSplinePerformance : MonoBehaviour {
		...
		void Start () {
			...
			float degree = 0f;
			int nodeLength = this.trackNodes + 1;// We need to add some extra because the first and last nodes just act as *guides* to the first and last curvature
			for(int i = 0; i < nodeLength; i++){
				float x = Mathf.Cos( degree * Mathf.Deg2Rad ) * this.circleLength + Random.Range(0f, this.randomRange);
				float z = Mathf.Sin( degree * Mathf.Deg2Rad ) * this.circleLength + Random.Range(0f, this.randomRange);
				randList.Add( new Vector3(x,1f,z ) );

				degree += 360f/(float)this.trackNodes;
			}
			...
			this.track = new LTSpline( randList.ToArray() );
			...
			Framework.LeanTween.moveSpline( this.trackTrailRenderers, this.track, this.tracerSpeed ).setOrientToPath(true).setRepeat(-1);
		}
		...
		void Update () {
			...
			if(this.trackPosition>1f)
				this.trackPosition = 0f; // We need to keep the ratio between 0-1 so after one we will loop back to the beginning of the track
		}
