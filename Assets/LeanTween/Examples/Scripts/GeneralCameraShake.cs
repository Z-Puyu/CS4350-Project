...
namespace LeanTween.Examples.Scripts {
	public class GeneralCameraShake : MonoBehaviour {
		...
		void Start () {
			...
			this.boomAudioClip = LeanAudio.createAudio(volumeCurve, frequencyCurve, LeanAudio.options().setVibrato( new Vector3[]{ new Vector3(0.1f,0f,0f)} ));
		...
		void bigGuyJump(){
			...
			Framework.LeanTween.moveY(this.avatarBig, height, 1f).setEase(LeanTweenType.easeInOutQuad).setOnComplete( ()=>{
				Framework.LeanTween.moveY(this.avatarBig, 0f, 0.27f).setEase(LeanTweenType.easeInQuad).setOnComplete( ()=>{
					Framework.LeanTween.cancel(this.gameObject);
				...
				**************/
					...
					LTDescr shakeTween = Framework.LeanTween.rotateAroundLocal( this.gameObject, Vector3.right, shakeAmt, shakePeriodTime)
					                              .setEase( LeanTweenType.easeShake ) // this is a special ease that is good for shaking
					                              .setLoopClamp()
					                              .setRepeat(-1);

					// Slow the camera shake down to zero
					Framework.LeanTween.value(this.gameObject, shakeAmt, 0f, dropOffTime).setOnUpdate( 
						(float val)=>{
