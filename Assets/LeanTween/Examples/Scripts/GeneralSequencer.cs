...
namespace LeanTween.Examples.Scripts {
	public class GeneralSequencer : MonoBehaviour {
		...
		public void Start(){
			...
			seq.append( Framework.LeanTween.moveY( this.avatar1, this.avatar1.transform.localPosition.y + 6f, 1f).setEaseOutQuad() );
			...
			seq.insert( Framework.LeanTween.alpha(this.star, 0f, 1f) );
			seq.insert( Framework.LeanTween.scale( this.star, Vector3.one * 3f, 1f) );

			// Rotate 360
			seq.append( Framework.LeanTween.rotateAround( this.avatar1, Vector3.forward, 360f, 0.6f ).setEaseInBack() );

			// Return to ground
			seq.append( Framework.LeanTween.moveY( this.avatar1, this.avatar1.transform.localPosition.y, 1f).setEaseInQuad() );

			// Kick off spiraling clouds - Example of appending a callback method
			seq.append(() => {
				for(int i = 0; i < 50f; i++){
					GameObject cloud = Object.Instantiate(this.dustCloudPrefab) as GameObject;
					...
					Framework.LeanTween.moveLocal(cloud, range, 3f*this.speedScale).setEaseOutCirc();
					Framework.LeanTween.rotateAround(cloud, Vector3.forward, 360f*2, 3f*this.speedScale).setEaseOutCirc();
					Framework.LeanTween.alpha(cloud, 0f, 3f*this.speedScale).setEaseOutCirc().setDestroyOnComplete(true);
				}
			});
