...
namespace LeanTween.Testing {
	public class TestingMaxTweens : MonoBehaviour {
		...
		void Awake(){
			Framework.LeanTween.init (20);
		}

		void Update(){
			//		Debug.Log ("tweenIter:" + tweenIter + " tweensRunning:" + LeanTween.tweensRunning + " Time:" + Time.time);
			if (this.tweenIter < 20) {
				GameObject box = GameObject.CreatePrimitive(PrimitiveType.Cube);
				Object.Destroy( box.GetComponent( typeof(BoxCollider) ) as Component );

				//			Debug.Log ("new Time.time:" + Time.time);
				Framework.LeanTween.moveX (box, 100f, 2f).setUseEstimatedTime(true).setOnComplete( ()=>{
